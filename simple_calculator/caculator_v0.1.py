...
def lambda_handler(event, context):
    ...
    if (event["session"]["application"]["applicationId"] !=
            "amzn1.ask.skill.f7ba7e91-1835-414c-bf57-1c32d80c1b8c"):
...
def Add(intent):
    print (intent)
    session_attributes = {}
    operaters = ['subtract', 'add']
    value_string = intent['slot']['number']['value']
    value_list_raw = value_string.split(' ')
    ...
    if not set(operators).intersection(set(value_list)):
        speech_output = "Your expression need subtract or add"
    ...
    speech_output = "one plus one equal two. "
